$(function(){
	...
	textArea.on('keydown keyup keypress change focus blur click',function(){//keydown keyup keypress change: 入力エリアになにかしらの反応があった時のアクションを記述
		...
		setText = selfVal.replace(/^[^\x20-\x7e]*$ +^[^\x20-\x7e]*$/g, '$1$2').replace(/^[^\x20-\x7e]*$ +^[^\x20-\x7e]*$/g, '$1$2').replace(/[^\x01-\x7E] +([\w.])/g, '$1$2').replace(/[^\x01-\x7E],+([^\w.])/g, '$1、$2').replace(/[^\x01-\x7E]，+([^\w.])/g, '$1、$2').replace(/^[^\x20-\x7e]*$\.+/g, '$1。').replace(/^[^\x20-\x7e]*$\．+/g, '$1。').replace(/[^\x01-\x7E]、+([\w.])/g, '$1,$2').replace(/([\w.])。+/g, '$1.').replace(/[^\x01-\x7E]:+([\w.])/g, '$1: $2');//半角スペースの繰り返しを消す //([^\w.])は全角 //+は直前の1回以上の繰り返し //カッコ()が＄の番号
		self.val(setText);//全角(/^[^\x20-\x7e]*$/)、半角(/[^\x01-\x7E]/)
	});
});
